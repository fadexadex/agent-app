...
<!-- Image generation tool section commented out for demo speed
...
When the user message contains `[PROJECT_CONTEXT: projectId="<id>"]`:
- Extract the `projectId` value from between the quotes
- **Always** pass it as the `projectId` argument to `renderScene`
- This organises each scene's rendered videos under the project's folder so versions are tracked correctly
